...
## the output is "TC_tc_Model_IV_outputs.Rdata" where tc should be replaced by appropriate TC number from 1, 2 and 3 that is a list fill_up_missing_data containing four components:
...
data = coarse_creation$coarse_data[ , , TC]	## data representing the corresponding TC measurement chosen in the previous line where rows and columns represents gridcells and months respectively
...
data_TC1 = coarse_creation$coarse_data[ , , 1]	## TC1 measurement

pos_0_cell = apply(data, 1, function(x) which(x == 0))	## list of all missing months at each cell

pos_0_cell_length = sapply(pos_0_cell, length)	## number of missing months at each cell

pos_0_month = apply(data, 2, function(x) which(x == 0))	## list of all missing cells at each month

pos_0_month_length = sapply(pos_0_month, length)	## number of missing cells at each months

pos_0 = which(data == 0) ## vector of all missing data 

## Translating and rescaling available data to the same range as that of TC1, and then performing log transformation
...
location = coarse_creation$coarse_location	## location of each cell

S = dim(location)[1]	## Total number of gridcells

T = dim(data)[2]	## Total number of months 

n_total = S*T	## Total number of data points

n_non_missing = n_total - length(pos_0)	## Total number of available data points
...
vec = which((pos_0_month_length/S) < 0.5)	## Months at which 50% or more data are available. Moran's I is calculated only for these months 
...
phi_store = array(0, dim = c(S, T, nmc/nthin))

tau2_store = matrix(0, T, nmc/nthin)

psi_store = matrix(0, T, nmc/nthin)

psi_0_store = matrix(0, 1, nmc/nthin)

eta_store = matrix(0, 1, nmc/nthin)

kappa2_store = matrix(0, 1, nmc/nthin)

pred_store = matrix(0, length(pos_0), nmc/nthin)	## prediction of missing data
...
for (iter in 1:(nit+nmc))
	...
	## posterior distribution of phi and tau2
    ...
    if (iter > nit & (iter - nit)%%nthin==0)
		{
			...
			pred_store[ , (iter - nit)/nthin] = (sweep( (- y_minus_X_beta  + phi), 2, (-psi)))[pos_0] + rnorm(length(pos_0), 0, sqrt(sigma2))
	  
			nlog_likelihood_store[(iter - nit)/nthin] = n_non_missing*log(sigma2)/2 + sum( (sweep((y_minus_X_beta - phi), 2, psi))[-pos_0]^2)/(2*sigma2)
...
if(TC == 3) 
	...
	mean_phi = apply(phi_store, c(1, 2), mean)
	
	mean_psi = apply(psi_store, 1, mean)
	
	mean_X_beta = colSums(t(covariate) * mean_beta)
	
	mean_res = sweep((data - mean_phi), 2, (mean_X_beta + mean_psi))
	...
	for (i in 1:(nmc/nthin) ) residual_store[ , , i] = sweep((data - phi_store[ , , i]) , 2 , (colSums(t(covariate) * beta_store[ , i]) + psi_store[ , i] )) 
	...
	save(fill_up_missing_data, file = paste0("Inputs_and_Outputs/Model_IV_TC_", TC ,"_outputs.Rdata"))