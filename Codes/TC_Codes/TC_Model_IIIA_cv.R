...
## the output is "CV_outputs_Model_IIIA_TC_tc.csv" where tc should be replaced by appropriate TC number from 1, 2 and 3 that is a 3x1 matrix containing mean of absolute bias, uncertainty and empirical coverage, repspectively.
...
for (iter in 1:(nit+nmc))
	...
	## posterior distribution of tau2 given others is Inverse Gamma
	...
	## posterior distribution of eta given others is Truncated Normal
    ...
    if (iter > nit & (iter - nit)%%nthin==0)
		{
			pred_test_store[ , (iter - nit)/nthin] = t(t(- y_minus_X_beta  + phi) + psi) [test_pos] + rnorm(l_test, 0, sqrt(sigma2))			
		}
}
...
write.csv(CV_outputs, file = paste0("Inputs_and_Outputs/Model_IIIA_TC_", TC, "_CV_outputs.csv"), row.names = FALSE)