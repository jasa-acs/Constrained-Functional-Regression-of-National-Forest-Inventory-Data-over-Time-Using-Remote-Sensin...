...
## the output is "CV_outputs_Model_II_TC_tc.csv" where tc should be replaced by appropriate TC number from 1, 2 and 3 that is a 3x1 matrix containing mean of absolute bias, uncertainty and empirical coverage, repspectively.
...
X_train = covariate[rep(1:T, (10000- pos_0_month_length)),]	## Construction of full covariate matrix for available TC feature
...
for (iter in 1:(nit+nmc))
	...
	if (iter > nit & (iter - nit)%%nthin==0)
		{
			pred_test_store[ , (iter - nit)/nthin] = t(t(- y_minus_X_beta) + psi) [test_pos] + rnorm(l_test, 0, sqrt(sigma2))			
...
write.csv(CV_outputs, file = paste0("Inputs_and_Outputs/Model_II_TC_", TC, "_CV_outputs.csv"), row.names = FALSE)