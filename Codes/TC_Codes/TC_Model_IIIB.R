...
## the output is "TC_tc_Model_IIIB_outputs.Rdata" where tc should be replaced by appropriate TC number from 1, 2 and 3 that is a list fill_up_missing_data containing four components:
...
pred_store = matrix(0, length(pos_0), nmc/nthin)
...
for (iter in 1:(nit+nmc))
	...
	## posterior distribution of sigma2
		...
		sum_S2 = sum((y_minus_X_beta[-pos_0] - phi[-pos_0])^2)
	...
	## piosterior distribution of phi and tau2
		...
		phi_gen = phi_generate_Model_IIIB(10)
    ...
    if (iter > nit & (iter - nit)%%nthin==0)
    {
		...
		pred_store[ , (iter - nit)/nthin] = - y_minus_X_beta[pos_0]  + phi[pos_0] + rnorm(length(pos_0), 0, sqrt(sigma2))
	  
		nlog_likelihood_store[(iter - nit)/nthin] = n_non_missing*log(sigma2)/2 + sum((y_minus_X_beta[-pos_0] - phi[-pos_0])^2)/(2*sigma2)
...
if(TC == 3) 
	...
	mean_res = sweep(data, 2, mean_X_beta) - mean_phi
	...
	for (i in 1:(nmc/nthin) ) residual_store[ , , i] = sweep(data , 2 , colSums(t(covariate) * beta_store[ , i]) ) - phi_store[ , , i]
	...
	save(fill_up_missing_data, file = paste0("Inputs_and_Outputs/Model_IIIB_TC_", TC ,"_outputs.Rdata"))
