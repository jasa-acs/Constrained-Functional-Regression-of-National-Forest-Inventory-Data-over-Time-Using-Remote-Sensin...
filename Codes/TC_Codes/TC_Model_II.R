...
## the output is "TC_tc_Model_II_outputs.Rdata" where tc should be replaced by appropriate TC number from 1, 2 and 3 that is a list fill_up_missing_data containing four components:
...
for (iter in 1:(nit+nmc))
	...
	## posterior distribution of beta is Multivariate Normal
		...
		y_train = t(t(data) - psi)
	
		beta_post_dispersion = inv_and_logdet.sym((XTX/sigma2)+ diag(p_beta)/c0)[[2]]     

		beta_post_mean =  crossprod(t(beta_post_dispersion),((crossprod(X_train, y_train[-pos_0])/sigma2)+(beta0/c0)))        
	...
	## posterior distribution of sigma2 is Inverse Gamma
		...
		sum_S2 = sum((t(t(y_minus_X_beta) - psi)[-pos_0])^2)
	...
	## posterior distribution of kappa2 is Inverse Gamma
	...
	## posterior distribution of psi is Multivariate Normal
		...
		for (t in setdiff(1:T, union(month_all, month_missing))) psi_mean_temp[t,] = mean( y_minus_X_beta[-pos_0_month[[t]],t] )
	...
	if (iter > nit & (iter - nit)%%nthin==0)
    {
		...
		res_temp = t(t(y_minus_X_beta) - psi)
		
		nlog_likelihood_store[(iter - nit)/nthin] = n_non_missing*log(sigma2)/2 + sum((res_temp[-pos_0])^2)/(2*sigma2)		

		pred_store[(iter - nit)/nthin, ] =  t(t(- y_minus_X_beta) + psi) [pos_0] + rnorm(length(pos_0), 0, sqrt(sigma2))
...
if(TC == 3) 
	...
	mean_X_beta = c(crossprod(t(covariate), mean_beta))
	
	mean_y_minus_X_beta = t(t(data) - mean_X_beta)

	mean_res = t(t(mean_y_minus_X_beta) - mean_psi)
	...
	for (i in 1:(nmc/nthin) ) residual_store[ , , i] = sweep(data , 2 , (colSums(t(covariate) * beta_store[ i , ]) + psi_store[i , ] ) ) 
	...
	save(fill_up_missing_data, file = paste0("Inputs_and_Outputs/Model_II_TC_", TC ,"_outputs.Rdata"))